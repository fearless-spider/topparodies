"""Django Sites N-gine Pages functions related to the ``request`` object."""
...
def get_template_from_request(request, page=None):
    ...
    if len(page_templates) == 0:
        return settings.SITESNGINE_PAGE_DEFAULT_TEMPLATE
    template = request.REQUEST.get('template', None)
    if template is not None and \
            (template in dict(page_templates).keys() or
                     template == settings.SITESNGINE_PAGE_DEFAULT_TEMPLATE):
        return template
    if page is not None:
...
def get_language_from_request(request):
    ...
    if hasattr(request, 'LANGUAGE_CODE'):
        lang = settings.SITESNGINE_PAGE_LANGUAGE_MAPPING(str(request.LANGUAGE_CODE))
        if lang not in LANGUAGE_KEYS:
            return settings.SITESNGINE_PAGE_DEFAULT_LANGUAGE
        else:
            return lang
    else:
        return settings.SITESNGINE_PAGE_DEFAULT_LANGUAGE
