# -*- coding: utf-8 -*-
"""A collection of functions for Sites N-gine Pages"""
import json
...
JSON_PAGE_EXPORT_NAME = 'sitesngine_page_export_version'
...
def export_po_files(path='poexport', stdout=None):
    ...
    source_language = settings.SITESNGINE_PAGE_DEFAULT_LANGUAGE
    source_list = []
    ...
    for lang in settings.SITESNGINE_PAGE_LANGUAGES:
        if lang[0] != settings.SITESNGINE_PAGE_DEFAULT_LANGUAGE:
            try:
                os.mkdir(path)
            except OSError:
                pass
...
def json_to_pages(json, user, preferred_lang=None):
    ...
    if not preferred_lang:
        preferred_lang = settings.SITESNGINE_PAGE_DEFAULT_LANGUAGE
...
def validate_pages_json_data(d, preferred_lang):
    ...
    seen_complete_slugs = dict(
        (lang[0], set()) for lang in settings.SITESNGINE_PAGE_LANGUAGES)

    valid_templates = set(t[0] for t in settings.get_page_templates())
    valid_templates.add(settings.SITESNGINE_PAGE_DEFAULT_TEMPLATE)
...
def monkeypatch_remove_pages_site_restrictions():
    ...
    """
    from sitesngine.pages.managers import PageManager
