# -*- coding: utf-8 -*-
"""Django Sites N-gine Pages ``managers``."""
...
from sitesngine.pages.utils import normalize_url
from sitesngine.pages.http import get_slug
...
class PageManager(TreeManager):
    ...
    def populate_pages(self, parent=None, child=5, depth=5):
        ...
        Content(body='page-' + str(p.id), type='title',
                language=settings.SITESNGINE_PAGE_DEFAULT_LANGUAGE, page=p).save()
    ...
    def from_path(self, complete_path, lang, exclude_drafts=True):
        ...
        if len(pages_list) == 1:
            if (settings.SITESNGINE_PAGE_USE_STRICT_URL and
    ...
    def create_and_update_from_json_data(self, d, user):
        ...
        page_languages = set(lang[0] for lang in settings.SITESNGINE_PAGE_LANGUAGES)
                ...
                for site in d['sites']:
                    try:
                        ...
                        messages.append(_("Could not add site '%s' to page")
                                        % (site,))
            if not settings.SITESNGINE_PAGE_HIDE_SITES and not page.sites.count():
...
class ContentManager(models.Manager):
    ...
    def set_or_create_content(self, page, language, ctype, body):
        ...
        """
        if settings.SITESNGINE_PAGE_SANITIZE_USER_INPUT:
    ...
    def create_content_if_changed(self, page, language, ctype, body):
        ...
        if settings.SITESNGINE_PAGE_CONTENT_REVISION_DEPTH:
            oldest_content = self.filter(page=page, language=language,
                                         type=ctype).order_by('-creation_date'
            )[settings.SITESNGINE_PAGE_CONTENT_REVISION_DEPTH:]
    ...
    def get_content(self, page, language, ctype, language_fallback=False):
        ...
        if not content_dict:
            content_dict = {}
            for lang in settings.SITESNGINE_PAGE_LANGUAGES:
                try:
...
class PageAliasManager(models.Manager):
    ...
    def from_path(self, request, path, lang):
        ...
        :param lang: not used
        """
        from sitesngine.pages.models import PageAlias
