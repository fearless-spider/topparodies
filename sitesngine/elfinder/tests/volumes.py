...
from sitesngine.elfinder.volumes.filesystem import ElfinderVolumeLocalFileSystem
from sitesngine.elfinder.volumes.storage import ElfinderVolumeStorage
...
class ElfinderVolumeLocalFileSystemTestCase(unittest.TestCase):
    ...
    def test_stat_file(self):
        stat = self.driver.stat(
            self.driver._join_path(self.driver._options['path'], self.driver._join_path('files', '2bytes.txt')))
        self.assertEqual(stat['name'], '2bytes.txt')
        self.assertEqual(stat['read'], 1)
        self.assertEqual(stat['write'], 1)
        self.assertEqual(stat['mime'].startswith('text/'), True)
        self.assertEqual(stat['size'], 2)
        self.assertEqual(stat['hash'], self.driver.encode(
            self.driver._join_path(self.options['path'], self.driver._join_path('files', '2bytes.txt'))))
    ...
    def test_dimensions(self):
        dim = self.driver.dimensions(self.driver.encode(self.driver._join_path(self.driver._options['path'],
                                                                               self.driver._join_path('files',
                                                                                                      self.driver._join_path(
                                                                                                          'directory',
                                                                                                          'yawd-logo.png')))))
    ...
    def test_tree(self):
        ...
        self.assertEquals(tree[1]['hash'],
                          self.driver.encode(self.driver._join_path(self.driver._options['path'], 'files')))
        self.assertEquals(tree[2]['hash'],
                          self.driver.encode(self.driver._join_path(self.driver._options['path'], 'test')))

    def test_open_close(self):
        hash_ = self.driver.encode(
            self.driver._join_path(self.options['path'], self.driver._join_path('files', '2bytes.txt')))
    ...
    def test_locked(self):
        stat = self.driver.stat(
            self.driver._join_path(self.options['path'], self.driver._join_path('files', 'directory')))
...
class ElfinderVolumeFilePermissionTestcase(unittest.TestCase):
    ...
    def test_read(self):
        self.assertEqual(
            self.driver._attr('%(root)s%(sep)sa%(sep)sb%(sep)sc' % {'root': self.root, 'sep': os.sep}, 'read'), False)
        self.assertEqual(
            self.driver._attr('%(root)s%(sep)sa%(sep)stmp%(sep)sc' % {'root': self.root, 'sep': os.sep}, 'read'), True)
        self.assertEqual(
            self.driver._attr('%(root)s%(sep)sb%(sep)stmp%(sep)sc' % {'root': self.root, 'sep': os.sep}, 'read'), False)
